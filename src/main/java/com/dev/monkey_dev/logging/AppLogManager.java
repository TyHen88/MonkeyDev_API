...
public class AppLogManager {
    ...
    private static String[] parseCaller(String str, Object x) {
        return new String[] {
                str,
                MessageFormat.format("{0}:{1} - {2}", StringUtils.substringAfterLast(str, ".") + ".java",
                        Thread.currentThread().getStackTrace()[4].getLineNumber(), String.valueOf(x))
        };
    }
    ...
    // error(getLogger(caller), MessageFormat.format("{0}:{1} - {2}", parsed[1],
    // Thread.currentThread().getStackTrace()[2].getLineNumber(), ex.getMessage()),
    // ex);
    // }
